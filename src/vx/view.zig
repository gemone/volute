...
pub fn render(self: *Editor) !void {
    ...
    } else if (self.mode == .normal and cursor_moved) {
        buf_ptr.render_cache.markDirty(self.last_render_cursor.row);
        buf_ptr.render_cache.markDirty(self.cursor.row);
    ...
    try term.moveTo(gpa, &buf, status_row, 0);
    ...
    const backend = switch (buf_ptr.backend()) {
        .gap_buffer => "gap",
        .tree_rope => "rope",
        .auto => "auto",
    };
    ...
    const pad_len = cols -| left.len -| right.len -| 1;
    try buf.appendSlice(gpa, left);
    for (0..pad_len) |_| try buf.appendSlice(gpa, " ");
    try buf.appendSlice(gpa, right);
    try term.setReverse(gpa, &buf, false);
    try buf.appendSlice(gpa, "\x1b[K");

    const cmd_row = rows - 1;
    try term.moveTo(gpa, &buf, cmd_row, 0);
    if (self.in_command_mode) {
        try term.setFg(gpa, &buf, .yellow);
        try buf.appendSlice(gpa, ":");
        try buf.appendSlice(gpa, self.command_buf.items);
        try term.resetAttrs(gpa, &buf);
    } else if (self.in_search_mode) {
        try term.setFg(gpa, &buf, .yellow);
        const dir_label = if (self.search_direction == .forward) "/" else "?";
        try buf.appendSlice(gpa, dir_label);
        try buf.appendSlice(gpa, self.command_buf.items);
        try term.resetAttrs(gpa, &buf);
    } else if (self.in_char_pending) {
        try term.setFg(gpa, &buf, .cyan);
        const prompt = switch (self.pending_char_command orelse .no_op) {
            .find_next_char => "f",
            .find_till_char => "t",
            .find_prev_char => "F",
            .till_prev_char => "T",
        ...
        try buf.appendSlice(gpa, prompt);
        try buf.appendSlice(gpa, "> ");
        try term.resetAttrs(gpa, &buf);
    } else if (self.in_numeric_prompt) {
        try term.setFg(gpa, &buf, .cyan);
        const prompt = switch (self.pending_numeric_command orelse .no_op) {
            .goto_line => "gd",
            .goto_column => "g|",
            else => "#",
        };
        ...
        if (self.pending_trie_name.len > 0) {
            try buf.appendSlice(gpa, " · ");
            try buf.appendSlice(gpa, self.pending_trie_name);
        }
        try term.resetAttrs(gpa, &buf);
...
fn appendRenderedTextRow(
    ...
    const buf_ptr = self.getBuffer() orelse return;
    const text_start = contentStartColumn(line_num_width);
    const line_idx = self.scroll + screen_row;

    // Skip rendering if line is clean and hasn't changed
    const cursor_line_active = self.mode == .normal and line_idx == self.cursor.row;
    if (!force_draw and !cursor_line_active and !buf_ptr.render_cache.isDirty(line_idx) and self.mode != .select_) {
        // Skip this line - it's already rendered correctly
        return;
    }

    try term.moveTo(gpa, buf, screen_row, 0);

    if (line_idx < buf_ptr.lineCount()) {
        try term.setFg(gpa, buf, .gray);
        var num_buf: [16]u8 = undefined;
        const line_num = std.fmt.bufPrint(&num_buf, "{d}", .{line_idx + 1}) catch "";
        var pad: usize = 0;
        while (pad + line_num.len < line_num_width) : (pad += 1) {
            try buf.appendSlice(gpa, " ");
        }
        try buf.appendSlice(gpa, line_num);
        ...
        if (visible.len == 0 and line_idx == self.cursor.row and self.mode == .normal) {
            ...
            try appendStyledText(self, gpa, buf, line_idx, visible, style_buf.items);
        }

        try term.resetAttrs(gpa, buf);
    } else {
        try term.setFg(gpa, buf, .gray);
        for (0..line_num_width) |_| {
            try buf.appendSlice(gpa, "~");
        }
        try term.resetAttrs(gpa, buf);
        try buf.appendSlice(gpa, "   ");
...
fn lineWidth(line_count: usize) usize {
    return @max(3, std.fmt.count("{d}", .{line_count}));
}

fn contentStartColumn(line_num_width: usize) usize {
    return line_num_width + 3;
}
...
fn applyCharStyle(
    ...
    switch (style) {
        .normal => {},
        .keyword => {
            try term.setFg(gpa, buf, .magenta);
            try term.setBold(gpa, buf, true);
        },
        .type_name => {
            try term.setFg(gpa, buf, .cyan);
            try term.setBold(gpa, buf, true);
        },
        .string => try term.setFg(gpa, buf, .green),
        .comment => try term.setFg(gpa, buf, .gray),
        .number => try term.setFg(gpa, buf, .yellow),
        .builtin => try term.setFg(gpa, buf, .blue),
    }
    if (reversed) try term.setReverse(gpa, buf, true);
}
...
fn appendStyledText(
    ...
    const cursor_byte = if (line_idx == self.cursor.row and self.mode == .normal)
        normalCursorByte(line, self.cursor.col)
    else
        null;
    ...
    while (iter.nextCodepointSlice()) |slice| {
        ...
        const in_selection = if (self.selection) |sel|
            @import("editor.zig").selectionContainsChar(sel, line_idx, start, line.len)
        else
            false;
        const reversed = (cursor_byte != null and cursor_byte.? == start) or in_selection;
        const style = styles[start];
        if (last_style == null or last_style.? != style or last_reversed != reversed) {
            try applyCharStyle(gpa, buf, style, reversed);
            last_style = style;
...
fn visibleByteCountForCells(line: []const u8, max_cells: usize) usize {
    ...
    var iter = std.unicode.Utf8View.initUnchecked(line).iterator();
    while (iter.nextCodepointSlice()) |slice| {
        const width = codepointCellWidth(std.unicode.utf8Decode(slice) catch std.unicode.replacement_character);
...
fn cursorScreenColumn(buf: *Buffer, row: usize, col: usize, max_cells: usize) usize {
    const line = buf.getLine(row) orelse return 0;
    return @min(displayCellsToColumn(line, col), max_cells);
}
...
fn appendCursorPresentation(
    ...
    try term.setCursorStyle(gpa, buf, style);

    const cursor_screen_row = self.cursor.row -| self.scroll;
    try term.moveTo(
        gpa,
        buf,
        cursor_screen_row,
        text_start + cursorScreenColumn(buf_ptr, self.cursor.row, self.cursor.col, cols -| text_start),
    );
}

fn displayCellsToColumn(line: []const u8, col: usize) usize {
    const target = alignUtf8Column(line, col, true);
    var used_cells: usize = 0;
    var iter = std.unicode.Utf8View.initUnchecked(line).iterator();
    while (iter.nextCodepointSlice()) |slice| {
        const start = iter.i - slice.len;
...
fn normalCursorByte(line: []const u8, col: usize) ?usize {
    ...
    const aligned = alignUtf8Column(line, col, true);
    if (aligned >= line.len) return prevUtf8Boundary(line, line.len);
    return aligned;
}

fn alignUtf8Column(line: []const u8, col: usize, allow_eol: bool) usize {
    return utf8.boundary(line).alignColumn(col, allow_eol);
}

fn prevUtf8Boundary(line: []const u8, col: usize) usize {
    return utf8.boundary(line).prev(col);
}

fn codepointCellWidth(codepoint: u21) usize {
...
test "appendRenderedTextRow keeps Chinese UTF-8 bytes in output" {
...
test "display cells use terminal width for CJK glyphs" {
    try std.testing.expectEqual(@as(usize, 2), displayCellsToColumn("你a", 3));
    try std.testing.expectEqual(@as(usize, 3), displayCellsToColumn("你a", 4));
...
test "appendRenderedTextRow keeps cursor styling outside Chinese UTF-8 bytes" {
    var editor = try initTestEditor("你a");
    defer deinitTestEditor(&editor);
    editor.mode = .normal;
    ...
    try std.testing.expect(std.mem.indexOf(u8, out.items, "\xbd\x1b") == null);
}

test "appendRenderedTextRow shows a cursor marker on an empty normal-mode line" {
...
test "appendCursorPresentation uses beam cursor in insert mode" {
    var editor = try initTestEditor("abc");
    defer deinitTestEditor(&editor);
    editor.mode = .insert;
    ...
    const buf_ptr = editor.getBuffer().?;
    try appendCursorPresentation(&editor, std.testing.allocator, &out, buf_ptr, 5, 80);

    try std.testing.expect(std.mem.indexOf(u8, out.items, "\x1b[6 q") != null);
}

test "appendCursorPresentation uses block cursor in normal mode" {
    var editor = try initTestEditor("abc");
    defer deinitTestEditor(&editor);
    editor.mode = .normal;
    editor.cursor = .{ .row = 0, .col = 1 };
    ...
    try std.testing.expect(std.mem.indexOf(u8, out.items, "\x1b[2 q") != null);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "\x1b[1;7H") != null);
}

test "appendCursorPresentation uses configured underline cursor in insert mode" {
    var editor = try initTestEditor("abc");
    defer deinitTestEditor(&editor);
    editor.mode = .insert;
    editor.insert_cursor_style = .underline;
    ...
    try std.testing.expect(std.mem.indexOf(u8, out.items, "\x1b[4 q") != null);
}

test "contentStartColumn accounts for full line-number gutter" {
    try std.testing.expectEqual(@as(usize, 6), contentStartColumn(3));
    try std.testing.expectEqual(@as(usize, 7), contentStartColumn(4));
}
